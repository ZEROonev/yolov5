...
from utils.datasets import create_dataloader1
...
def save_one_json(i,predn, jdict, path, class_map):
...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader1(data[task], imgsz, batch_size, gs, single_cls, pad=0.5, rect=True,
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for si, pred in enumerate(out):
            ...
            if save_json:
                save_one_json(i,predn, jdict, path, class_map)  # append to COCO-JSON dictionary
    ...
    if save_json and len(jdict):
        ...
        anno_json = './eval_city/val_gt.json'  # annotations json
        #pred_json = str(save_dir / f"{w}_predictions.json")  # predictions json
        print(f'\nEvaluating MR-2. saving ..')
        with open('./pred_json.json', 'w') as f:
            json.dump(jdict, f)
        from eval_city.eval_script.eval_demo import validate
        MRs = validate('./eval_city/val_gt.json', './pred_json.json')
        #print('MRS:',MRs)
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            eval.accumulate()
            eval.summarize()
            Reasonable,Bare,Partial,Heavy = eval.stats[:4]
            print(Reasonable,Bare,Partial,Heavy)  # update results (mAP@0.5:0.95, mAP@0.5)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (MRs[0],MRs[1],MRs[2],MRs[3], *(loss.cpu() / len(dataloader)).tolist()), maps, 


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default='data/citypersons.yaml', help='dataset.yaml path')
